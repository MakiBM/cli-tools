...
async function interactiveInstall(): Promise<void> {
  ...
  if (r.replaced) {
    console.log(pc.yellow('  ! Replaced an existing commit-msg hook — backup at <path>.pre-hideai'));
  }
  if (picked.length === 0) {
    console.log(pc.yellow('  ! Block list is empty — nothing will be blocked. Re-run to pick agents.'));
  } else {
