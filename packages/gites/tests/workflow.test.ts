...
test('start-feature creates feature + gitpace-<name>, defers origin push until ship', async () => {
  await withSandbox(async ({ remote }) => {
    ...
    const { runSetup } = await importFresh<typeof import('../src/setup.js')>('setup.ts');
    runSetup(remote, setupOpts());
    const { startFeature } = await importFresh<typeof import('../src/start-feature.js')>(
      'start-feature.ts',
    );
...
test('feature discovery lists pairs and active', async () => {
  await withSandbox(async ({ remote }) => {
    ...
    const { listFeatures, activeFeature } = await importFresh<typeof import('../src/feature.js')>(
      'feature.ts',
    );
