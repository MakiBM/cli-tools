...
test('workCutPoint finds the squash-merged parent boundary', async () => {
  await withSandbox(() => {
    ...
    const cut = workCutPoint('b', 'gitpace-b');
...
test('workCutPoint returns null when nothing is already merged', async () => {
  await withSandbox(() => {
    ...
    commitFile('x.txt', 'x\n', 'feat: work');
    assert.equal(workCutPoint('feat', 'gitpace-feat'), null);
  });
});
...
test('rebase --onto cut strips the parent with no conflict', async () => {
  await withSandbox(() => {
    ...
    const cut = workCutPoint('b', 'gitpace-b');
    assert.ok(cut);
    run('git', ['checkout', 'gitpace-b']);
    run('git', ['rebase', '--onto', 'b', cut!, 'gitpace-b']);

    const shipRange = run('git', ['log', '--format=%s', 'b..gitpace-b']).trim().split('\n');
