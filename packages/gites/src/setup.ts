...
export async function setupWizard(): Promise<void> {
  ...
  console.log(
    pc.dim("  Give each new feature its own worktree so you can work on several at once."),
  );
  const worktree = await confirm({
  ...
  console.log(
    pc.dim(
      `  Installs a pre-push hook into this clone, blocks '${remoteName}/*' branches from reaching origin.`,
    ),
  );
