...
export function shipCandidateShas(live: string, work: string): string[] {
  return gitTry('log', '--reverse', '--format=%H', '--cherry-pick', '--right-only', `${live}...${work}`)
    .split('\n')
    .filter(Boolean);
}
...
export async function ship(): Promise<void> {
  ...
  if (workDivergedFromLive(live, work)) {
    ...
    console.log(
      pc.dim(`Recover: git checkout ${work} && git reset --hard ${live}, then re-apply the delta as a new commit.`),
    );
    return;
  }
  ...
  // --- session window screen ----------------------------------------------
  {
    ...
    console.log(pc.dim(`Working hours ${fmt(WORK_START_MIN)}–${fmt(WORK_END_MIN)}, times ≥${MIN_GAP_MIN}m apart.`));
    console.log('');
    console.log(`  Start:   ${pc.cyan(`${localDate(startDT)} ${fmt(minutesOf(startDT))}`)}`);
    console.log(`  End:     ${pc.cyan(`${localDate(endDT)} ${fmt(minutesOf(endDT))}`)}  ${pc.dim('(now)')}`);
  ...
  for (let i = 0; i < count; i++) {
    console.log(`  ${schedule[i]!.date.slice(5)} ${schedule[i]!.time}  ${shas[i]!.slice(0, 8)}  ${subjects[i]}`);
  }
  console.log('');
  ...
  for (let i = 0; i < count; i++) {
    ...
    const cpOk = await withSpinner(
      `Cherry-picking ${sha.slice(0, 8)} as ${stamp}`,
    ...
    if (!cpOk) {
      ...
      console.log('');
      console.log(`Cherry-pick conflict on ${sha.slice(0, 8)} — reverted this ship, '${live}' is unchanged.`);
      console.log(`  '${work}' likely conflicts with what was already shipped; rebase it onto '${live}' and retry.`);
      failed = true;
      break;
    }
